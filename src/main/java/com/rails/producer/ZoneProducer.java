...
public class ZoneProducer {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("select * from gt10_zone ");
			List<Zone> zones = ResultSetHandler.getBeans(rs, Zone.class);
			zones.stream().forEach(e -> {
				MessageRequest mr = new MessageRequest();
				JSONObject JSe = new JSONObject();
				@SuppressWarnings("static-access")
				String jsonStringe = JSe.toJSONString(e);
				mr.setBody(jsonStringe);
				mr.setRequest_id("");
				MsgJson msgJson = new MsgJson();
				msgJson.setMessageRequest(mr);
				msgJson.setTags("*");
				msgJson.setTitle("酒店同步");
				msgJson.setTopic("Hotel_gt10_zone");
				JSONObject JS = new JSONObject();
				@SuppressWarnings("static-access")
				String jsonString = JS.toJSONString(msgJson);
				Message msgJ = new Message("Hotel_gt10_zone", "*", jsonString.getBytes());
